  ...
  const renderProductCard = (p, isTrash = false) => (
    <div key={p.id} className="bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-700 hover:border-blue-500 transition">
      ...
      <div className="p-4 space-y-3">
        <h3 className="text-xl font-bold text-white line-clamp-2">{p.productType}</h3>
        ...
        <div className="grid gap-2">
          ...
          {!isTrash && p.status !== "rejected" && <button onClick={() => reject(p.id)} className="bg-red-600 hover:bg-red-500 text-white py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2"><XCircle className="w-5 h-5" /> Цуцлах (зөвлөмжтэй)</button>}
        ...
        <div className="text-center pt-2">
          {!isTrash && p.status === "pending" && <span className="text-yellow-400 text-sm flex items-center gap-1 justify-center"><Clock className="w-4 h-4" /> Хүлээгдэж байна</span>}
          {!isTrash && p.status === "approved" && <span className="text-green-400 text-sm flex items-center gap-1 justify-center"><CheckCircle className="w-4 h-4" /> Зөвшөөрсөн</span>}
          {!isTrash && p.status === "rejected" && <span className="text-red-400 text-sm flex items-center gap-1 justify-center"><XCircle className="w-4 h-4" /> Цуцлагдсан</span>}
          {isTrash && <span className="text-purple-400 text-sm flex items-center gap-1 justify-center"><Trash2 className="w-4 h-4" /> Хогийн саванд</span>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white py-10">
      <div className="max-w-6xl mx-auto px-4">
        ...
        {tab === "pending" && renderTabContent(pending, <Clock className="w-12 h-12" />, "Шинэ бараа алга", "Бүгдийг шалгасан")}
        {tab === "approved" && renderTabContent(approved, <CheckCircle className="w-12 h-12" />, "Бүгд зөвшөөрсөн", "Хэрэглэгчдэд харагдана")}
        {tab === "rejected" && renderTabContent(rejected, <XCircle className="w-12 h-12" />, "Цуцлагдсан бараа байхгүй", "Малчин зөвлөмж харна")}
        {tab === "trash" && renderTabContent(deleted, <Trash2 className="w-12 h-12" />, "Хогийн сав хоосон", "Сэргээх эсвэл устгах боломжтой", true)}
      </div>
    </div>
  );
}